/**
 * Express Server using PDF2PIC for proper Windows path handling
 */
...
import { PDF2PicExtractor } from './extraction-pdf2pic';

const app = express();
const PORT = process.env.PORT || 3002;

// Use the PDF2PIC extractor
const extractionService = new PDF2PicExtractor();
...
app.get('/api/health', (req, res) => {
  res.json({ 
    status: 'healthy',
    version: '3.0',
    extraction: 'PDF2PIC + Vision API',
    message: 'Using pdf2pic for proper Windows path handling'
  });
});
...
const server = app.listen(PORT, () => {
  console.log(`🚀 Arkansas Contract Extractor Server (PDF2PIC Edition)`);
  console.log(`📍 Running at: http://localhost:${PORT}`);
  console.log(`✅ Using PDF2PIC for proper Windows path handling`);
