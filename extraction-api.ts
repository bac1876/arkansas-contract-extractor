/**
 * API Wrapper for Arkansas Contract Extraction
 * Provides a module interface for the Vision API extraction
 */
...
export class ContractExtractionAPI {
  ...
  /**
   * Extract data from a PDF contract
  ...
  async extractFromPDF(pdfPath: string): Promise<{
    ...
    try {
      ...
      // Use simple relative path - no nested folders to avoid path issues
      ...
      console.log('Output folder:', tempDir);
      
      // Convert to PNG - use simple relative path
      ...
      if (pngPages.length >= 1) {
        const page1Path = pngPages[0]?.path || path.join(tempDir, 'page1.png');
        const page1Data = await this.extractPage1(page1Path);
        Object.assign(extractedData, page1Data);
      }
      
      // Skip pages 2-3 (agency not needed)
      
      // Page 4 - Financial Terms
      if (pngPages.length >= 4) {
        const page4Path = pngPages[3]?.path || path.join(tempDir, 'page4.png');
      ...
      if (pngPages.length >= 5) {
        const page5Path = pngPages[4]?.path || path.join(tempDir, 'page5.png');
      ...
      if (pngPages.length >= 6) {
        const page6Path = pngPages[5]?.path || path.join(tempDir, 'page6.png');
      ...
      if (pngPages.length >= 7) {
        const page7Path = pngPages[6]?.path || path.join(tempDir, 'page7.png');
      ...
      if (pngPages.length >= 8) {
        const page8Path = pngPages[7]?.path || path.join(tempDir, 'page8.png');
      ...
      if (pngPages.length >= 10) {
        const otherData = await this.extractPages10to16(tempDir, pngPages.length, pngPages);
...
Return JSON with home_warranty, warranty_details, warranty_paid_by, inspection_option, inspection_details`
  ...
  private async extractPages10to16(tempDir: string, totalPages: number, pngPages?: any[]): Promise<Partial<ContractExtractionResult>> {
    const results: any = {};

    // Extract from each remaining page as needed
    // Page 10 - Wood/Termite
    if (totalPages >= 10) {
      try {
        const page10Path = pngPages?.[9]?.path || path.join(tempDir, 'page10.png');
    ...
    if (totalPages >= 12) {
      try {
        const page12Path = pngPages?.[11]?.path || path.join(tempDir, 'page12.png');
  ...
  /**
   * Convert extraction result to CSV format
   */
