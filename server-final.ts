/**
 * FINAL Express Server - Uses GPT-4 Vision API
 * Works around pdf-to-png-converter path issues
 */
...
import { DirectGPT4Extractor } from './extraction-gpt4-direct';

const app = express();
const PORT = process.env.PORT || 3005;

// Use the DIRECT GPT-4 Vision API extractor (actually calls the API!)
const extractionService = new DirectGPT4Extractor();
...
app.get('/api/health', (req, res) => {
  res.json({ 
    ...
    message: 'Final working version - no path issues!'
  });
});
...
app.post('/api/extract', upload.single('contract'), async (req, res) => {
  ...
  try {
    ...
    // Use final extraction with GPT-4 Vision API
...
const server = app.listen(PORT, () => {
  console.log(`🚀 Arkansas Contract Extractor Server (FINAL VERSION)`);
  console.log(`📍 Running at: http://localhost:${PORT}`);
  console.log(`✅ Using GPT-4 Vision API for extraction`);
  console.log(`🎯 No more path duplication issues!`);
  console.log(`📄 Ready to process contracts`);
});
