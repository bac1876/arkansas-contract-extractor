...
async function testPreconvertedImage() {
  const openai = new OpenAI({ 
    apiKey: process.env.OPENAI_API_KEY 
  });

  console.log('🔍 Testing GPT-4o with pre-converted image (that works locally)...\n');
  
  // Read the pre-converted image that the local script uses
  const imageBuffer = fs.readFileSync('contract2_pages/page1.png');
  const base64Image = imageBuffer.toString('base64');
  console.log('📏 Pre-converted image size:', imageBuffer.length, 'bytes\n');
  
  // Use the EXACT same prompt as the working local script
  ...
  try {
    ...
    try {
      ...
      if (parsed.buyers && parsed.buyers[0].includes('Brian')) {
        ...
        console.log('\n⚠️ WARNING: Returned demo data (John Doe)');
      }
    } catch (e) {
...
testPreconvertedImage().catch(console.error);