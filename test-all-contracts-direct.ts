...
async function testContract(filename: string): Promise<ContractTestResult> {
  ...
  try {
    ...
    // Extract with GPT-5-mini directly
...
async function main() {
  console.log('🚀 GPT-5-mini Direct Contract Testing');
  console.log('='.repeat(60));
  console.log('⚠️  Note: This will take several minutes per contract\n');
  
  // List of contracts to test (start with just a few)
  const contracts = [
    'test_contract2.pdf',  // Known working test
    'Offer (BBS)-269 Honor Court.pdf',
    'Offer (EXE)-3461 Alliance Dr.pdf',
    // Add more later if these work
  ];
  ...
  for (const contract of contracts) {
    try {
      ...
      // Add delay between contracts to avoid rate limits
  ...
  for (const result of results) {
    ...
    if (result.buyers && result.propertyAddress) {
      console.log(`   ${result.buyers.join(', ')} - ${result.propertyAddress}`);
    }
