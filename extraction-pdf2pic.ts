/**
 * PDF2PIC-based extraction module for Arkansas Contract Extraction
 * Uses pdf2pic library which properly handles Windows paths
 */
...
export class PDF2PicExtractor {
  ...
  async extractFromPDF(pdfPath: string): Promise<{
    ...
    try {
      console.log(`🔍 PDF2Pic Extractor: Starting extraction for: ${pdfPath}`);
      
      // Create a unique temp folder for this extraction
      const timestamp = Date.now().toString();
      const tempFolder = path.join(process.cwd(), `pdf2pic_temp_${timestamp}`);
      
      console.log(`📁 Creating temp folder: ${tempFolder}`);
      await fs.mkdir(tempFolder, { recursive: true });
      
      console.log('🖼️ Converting PDF to images using pdf2pic...');
      
      // Configure pdf2pic options
      const options = {
        density: 150,           // DPI for image quality
        saveFilename: 'page',   // Base filename for pages
        savePath: tempFolder,   // Output directory
        format: 'png',          // Output format
        width: 2550,            // Width for better OCR
        height: 3300            // Height for better OCR
      };
      
      // Initialize converter
      const converter = fromPath(pdfPath, options);
      
      // Get total pages (convert page 1 first to test)
      console.log('📄 Converting page 1 to determine total pages...');
      const firstPage = await converter(1);
      
      // Now convert all pages (pdf2pic uses 1-based indexing)
      console.log('📄 Converting all pages...');
      const pagePromises = [];
      
      // We'll convert up to 20 pages (Arkansas contracts are typically 16-17 pages)
      for (let i = 1; i <= 20; i++) {
        pagePromises.push(converter(i).catch(() => null));
      }
      
      const allPages = await Promise.all(pagePromises);
      const validPages = allPages.filter(p => p !== null);
      
      console.log(`✅ Converted ${validPages.length} pages successfully`);
      ...
      if (validPages.length >= 1) {
        const page1Path = path.join(tempFolder, 'page.1.png');
        const page1Data = await this.extractPage1(page1Path);
        Object.assign(extractedData, page1Data);
      }
      
      // Page 4 - Financial Terms
      if (validPages.length >= 4) {
        const page4Path = path.join(tempFolder, 'page.4.png');
      ...
      if (validPages.length >= 5) {
        const page5Path = path.join(tempFolder, 'page.5.png');
      ...
      if (validPages.length >= 6) {
        const page6Path = path.join(tempFolder, 'page.6.png');
      ...
      if (validPages.length >= 7) {
        const page7Path = path.join(tempFolder, 'page.7.png');
      ...
      if (validPages.length >= 8) {
        const page8Path = path.join(tempFolder, 'page.8.png');
      ...
      if (validPages.length >= 10) {
        const otherData = await this.extractPages10to16(tempFolder, validPages.length);
      ...
      console.error('❌ PDF2Pic Extractor Error:', error);
      return {
        success: false,
...
Return JSON with home_warranty, warranty_details, warranty_paid_by, inspection_option, inspection_details`
  ...
  private async extractPages10to16(tempFolder: string, totalPages: number): Promise<Partial<ContractExtractionResult>> {
    const results: any = {};

    // Page 10 - Wood/Termite
    if (totalPages >= 10) {
      try {
        const page10Path = path.join(tempFolder, 'page.10.png');
    ...
    if (totalPages >= 12) {
      try {
        const page12Path = path.join(tempFolder, 'page.12.png');
